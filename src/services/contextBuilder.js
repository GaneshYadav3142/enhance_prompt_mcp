...
// src/services/contextBuilder.js
// Builds the [Project context] block from session history.
// Supports OpenAI and Gemini for summary generation.

const config = require("../config");
const session = require("../session/manager");

// ── summary generation ────────────────────────────────────────────────────────
async function generateSummaryOpenAI(turns, previousSnapshot) {
  const OpenAI = require("openai");
  const client = new OpenAI({ apiKey: config.openaiApiKey });
...
async function generateSummaryGemini(turns, previousSnapshot) {
  const { GoogleGenerativeAI } = require("@google/generative-ai");
  const genAI = new GoogleGenerativeAI(config.geminiApiKey);
  const model = genAI.getGenerativeModel({ model: config.summariserModel });
  ...
  const prompt = `You are a project context tracker. Given a conversation, extract:
1. A 2-3 sentence summary of the project and what has been discussed
2. The current active task (one sentence)
...
${prevSummary}Recent conversation:
${history}`;
...
async function generateSummary(turns, previousSnapshot) {
  if (config.provider === "gemini") {
    return generateSummaryGemini(turns, previousSnapshot);
  }
  return generateSummaryOpenAI(turns, previousSnapshot);
}
...
async function buildContextBlock(sessionId) {
  ...
  const lastSummaryAt = snapshot?.turn_index ?? 0;
  const needsNewSummary = turns.length > 0 &&
    (turnCount - lastSummaryAt) >= config.summaryEveryNTurns;
